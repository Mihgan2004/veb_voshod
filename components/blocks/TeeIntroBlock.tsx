...
function IntroTitle({ className = '', style }: { className?: string; style?: React.CSSProperties }) {
  return (
    <motion.h2 className={className} style={style as MotionStyle}>
...
function IntroBody({ className = '', style }: { className?: string; style?: React.CSSProperties }) {
  return (
    <motion.div className={className} style={style as MotionStyle}>
...
function mkStaticStyle({
  ...
  const opacity = Math.round(enter * fadeOut * 1e5) / 1e5;
  const translateY = Math.round(lerp(slideFrom, 0, enter) * 100) / 100;
...
function useEnterStyle({
  ...
  if (isMobile) return { opacity: 1, transform: 'none' };
  const opacity = useTransform([enter, fadeOut], ([en, fo]: number[]) =>
    Math.round(en * fo * 1e5) / 1e5,
  );
  const y = useTransform(enter, (en: number) =>
    Math.round(lerp(slideFrom, 0, en) * 100) / 100,
  );
  return { opacity, y };
}

function TeeIntroBlockAnimated({
  sectionRef,
  isDesktop,
  compact,
  isMobile,
  ...
  const { scrollYProgress } = useScroll({
    target: sectionRef,
    // Эквивалент прежней формулы: 0 когда top==top, 1 когда bottom==bottom
    offset: ['start start', 'end end'],
  });

  // Чуть “подпираем” старт, чтобы интро не было полностью прозрачным при первом касании (как раньше effectiveP>=0.15 near start).
  const progress = useTransform(scrollYProgress, (v) => {
    const raw = clamp01(v);
    if (raw < 0.1) return Math.max(raw, 0.15);
    return raw;
  });

  const fadeOut = useTransform(progress, (p) => {
    const out = smoothstep(0.78, 0.98, p);
    return lerp(1, 0.35, out);
  });

  const backdropFade = useTransform(progress, (p) => {
    if (isMobile) return 1;
    return lerp(1, 0, smoothstep(0.68, 0.92, p));
  });

  const e0 = useTransform(progress, (p) => smoothstep(0, 0.12, p));
  const e1 = useTransform(progress, (p) => smoothstep(0.02, 0.15, p));
  const e2 = useTransform(progress, (p) => smoothstep(0.05, 0.19, p));
  const e3 = useTransform(progress, (p) => smoothstep(0.08, 0.23, p));
  const e4 = useTransform(progress, (p) => smoothstep(0.11, 0.27, p));

  const monoStyle = useEnterStyle({ isMobile, enter: e0, fadeOut, slideFrom: 20 });
  const titleStyle = useEnterStyle({ isMobile, enter: e1, fadeOut, slideFrom: 32 });
  const bodyStyle = useEnterStyle({ isMobile, enter: e2, fadeOut, slideFrom: 26 });
  const teeStyle = useEnterStyle({ isMobile, enter: e1, fadeOut, slideFrom: 16 });
  const cardsStyle = useEnterStyle({ isMobile, enter: e3, fadeOut, slideFrom: 36 });
  const ctaStyle = useEnterStyle({ isMobile, enter: e4, fadeOut, slideFrom: 24 });
            ...
            <div className="mx-auto flex w-full max-w-lg flex-col items-center gap-7 sm:gap-8">
              <div className="flex w-full flex-col items-center gap-5 text-center">
                ...
                <IntroTitle
                  ...
                  style={titleStyle as unknown as React.CSSProperties}
                />

                <IntroBody
                  className="max-w-[21rem] flex flex-col gap-3 text-[13px] sm:text-sm leading-[1.6] text-white/75"
                  style={bodyStyle as unknown as React.CSSProperties}
                />
              </div>
              ...
              <SpecCardsGrid
                ...
                style={cardsStyle as unknown as React.CSSProperties}
              />

              <motion.div className="flex w-full justify-center pt-2 sm:pt-4" style={ctaStyle}>
                <Link
                  href="/catalog"
            ...
            <div className="h-full grid grid-cols-12 items-center gap-6 sm:gap-8 lg:gap-12 xl:gap-16">
              ...
              <div className="col-span-12 md:col-span-8 flex flex-col justify-center items-center md:items-start md:mt-0 relative">
                ...
                <div className="relative z-10">
                  <motion.div className="text-[10px] sm:text-[11px] lg:text-[11px] font-mono tracking-[0.2em] lg:tracking-[0.24em] text-white/30 lg:text-white/35 mb-3 sm:mb-4 lg:mb-4" style={monoStyle}>
                    {INTRO_MONO}
                    {!animationsDisabled && (
                  ...
                  <motion.div className="mt-5 sm:mt-7 lg:mt-8" style={ctaStyle}>
...
function TeeIntroBlockStatic({
  ...
  const backdropFade = isMobile ? 1 : lerp(1, 0, smoothstep(0.68, 0.92, p));
  ...
  const monoStyle = mkStaticStyle({ isMobile, enter: e0, fadeOut, slideFrom: 20 });
  const titleStyle = mkStaticStyle({ isMobile, enter: e1, fadeOut, slideFrom: 32 });
  const bodyStyle = mkStaticStyle({ isMobile, enter: e2, fadeOut, slideFrom: 26 });
  const teeStyle = mkStaticStyle({ isMobile, enter: e1, fadeOut, slideFrom: 16 });
  const cardsStyle = mkStaticStyle({ isMobile, enter: e3, fadeOut, slideFrom: 36 });
  const ctaStyle = mkStaticStyle({ isMobile, enter: e4, fadeOut, slideFrom: 24 });
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden md:sticky md:top-0 md:min-h-screen">
        <div className="pointer-events-none absolute inset-0" style={{ opacity: backdropFade }} aria-hidden>
          <div className="absolute inset-0 bg-[var(--vx-bg-base)]" />
          <div
            className="hidden md:block absolute inset-x-0 bottom-0 h-[28%] bg-gradient-to-b from-transparent to-[var(--vx-bg-section)] pointer-events-none"
            aria-hidden
        ...
        <div className="relative z-10 max-w-7xl lg:max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-12 xl:px-16 py-8 pb-10 sm:py-10 md:py-0 md:min-h-screen md:h-full">
          {!isDesktop ? (
            <div className="mx-auto flex w-full max-w-lg flex-col items-center gap-7 sm:gap-8">
              <div className="flex w-full flex-col items-center gap-5 text-center">
                <div className="text-[10px] sm:text-[11px] font-mono tracking-[0.2em] text-white/30" style={monoStyle}>
                  <TypewriterLine disabled={false} />
                </div>
              ...
              <div className="relative w-full overflow-hidden py-0 pointer-events-none select-none" style={teeStyle} aria-hidden>
                <div
                  className="pointer-events-none absolute inset-0 z-0"
                  style={{
                    background: `
              ...
              <div className="flex w-full justify-center pt-2 sm:pt-4" style={ctaStyle}>
                <Link href="/catalog" className="vx-btn-primary w-full max-w-[280px] sm:max-w-[320px]">
                  <span>СМОТРЕТЬ КАТАЛОГ</span>
                  <span className="vx-btn-primary__arrow" aria-hidden>
                    →
                  </span>
            ...
            <div className="h-full grid grid-cols-12 items-center gap-6 sm:gap-8 lg:gap-12 xl:gap-16">
              ...
              <div className="col-span-12 md:col-span-8 flex flex-col justify-center items-center md:items-start md:mt-0 relative">
                <div
                  ...
                  style={{
                    background:
                      'linear-gradient(to right, rgba(5,6,7,0.72) 0%, rgba(5,6,7,0.22) 60%, transparent 100%)',
                  }}
                  aria-hidden
                />
                ...
                <div className="relative z-10">
                  <div
                    className="text-[10px] sm:text-[11px] lg:text-[11px] font-mono tracking-[0.2em] lg:tracking-[0.24em] text-white/30 lg:text-white/35 mb-3 sm:mb-4 lg:mb-4"
                    style={monoStyle}
                  >
                    {INTRO_MONO}
                    {!animationsDisabled && (
                      <span
                        className="inline-block w-[5px] h-[1em] bg-white/30 ml-1 align-middle"
                  ...
                  <div className="mt-5 sm:mt-7 lg:mt-8" style={ctaStyle}>
                    <Link href="/catalog" className="vx-btn-primary">
                      <span>СМОТРЕТЬ КАТАЛОГ</span>
                      <span className="vx-btn-primary__arrow" aria-hidden>
                        →
  ...
  if (effectiveCompact) {
    return (
      <TeeIntroBlockStatic
        ...
        liteMode={liteMode}
        p={noScrollOnMobile ? 0.15 : 0.85}
      />
    );
  }

  return (
    <TeeIntroBlockAnimated
      sectionRef={sectionRef}
      isDesktop={isDesktop}
      compact={compact}
      isMobile={isMobile}
