  ...
  return (
    <div className="animate-fade-in min-h-screen">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
        <div className="relative bg-graphite-light rounded-2xl border border-white/5 overflow-hidden">
          <img src={product.image} alt={product.name} className="w-full h-full object-cover min-h-[360px] lg:min-h-[640px]" />
        </div>
        ...
        <div>
          ...
          <div className="mt-10">
            <div className="text-xs font-mono uppercase tracking-widest text-gray-500 mb-4">SIZE</div>
              ...
              {product.sizes.map(size => (
                <button
                  ...
                  className={`w-14 h-12 border rounded-xl font-mono text-sm transition-all ${
                    selectedSize === size ? 'border-gold text-gold bg-gold/10' : 'border-white/10 text-gray-300 hover:border-white/20'
                  }`}
                >
                  {size}
                </button>
